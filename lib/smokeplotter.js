...
var SmokeChart = (function () {
    ...
    function SmokeChart() {
        ...
        this._yValueAverages = new Array();
    }
    ...
    SmokeChart.prototype = {
        ...
        PlotAverageEnabled: true,
        PlotDataEnabled: true,
        XLabels: new Array(),
        YInterval: -1.0,
        XLegendEnabled: true,
        YLegendEnabled: true,
        ...
        /**
         * Loads the data and prepares for charting
         * @param {} data -  the data to plot in two columns (x y)
         */
        LoadData: function (data) {
            ...
            // aggregate the data
            for (var i = 0; i < data.length; i++) {
                sum[data[i][0]] += parseFloat(data[i][1]);
                cnt[data[i][0]]++;
            }

            // calculate the average
            for (var key in sum) {
                this._yValueAverages[key] = sum[key] / cnt[key];
            }
    ...
    function DrawData() {
        ...
        for (var i = 1; i < this.Data.length; i++) {
            if (this.PlotDataEnabled)
                valueBox.Draw(this.Data[i][1], this._yValueAverages[this.Data[i][0]], this.Data[i][0]);
            if (this.PlotAverageEnabled)
                valueAverage.Draw(this._yValueAverages[this.Data[i][0]], this.Data[i][0]);
        }
    }
